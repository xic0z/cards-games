...
static int regra_valida_dest(Jogo *j, int orig, int dest, int qtd, RegraMove *r) {
    Pilha *po = &j->pilhas[orig];
    Pilha *pd = &j->pilhas[dest];
...
int logica_verificar_vitoria(Jogo *j) {
    ...
    for (int w = 0; w < j->n_wins && ok; w++) {
        ...
        for (int p = 0; p < j->n_pilhas && ok; p++) {
            if (j->pilhas[p].tipo_idx == tipo
                && j->pilhas[p].tam != esperado)
                ok = 0;
        }
    }
    return ok;
